...
**Mac 原生预览版 0.3.5** 已集成 PWA 0.3.4，面向 Apple silicon、macOS 14+，本机实际验收为 macOS 26。0.3.5 修复悬停预览移出后不关闭，并精简废纸篓与新建弹窗；此前已修复手机后台启动、连续预览，并新增紧凑 UI、笔记管理、固定块、AI 修改、原图内嵌与阅读工具。**跨 Windows 同步尚待真实跨机验证**，本次以独立本机使用为主。[下载 Mac 预览版](https://github.com/theincrediblewwz/MathNotes-OpenSource/releases/tag/macos-v0.3.5) · [Mac 发布说明](deploy/releases/macos-v0.3.5.md) · [接续开发与验收状态](apps/macos/RELEASE_STATUS.md)。
...
| macOS | 0.3.5 原生预览版 | Apple silicon ZIP，内置 PWA；ad-hoc 签名，未公证 |

本版提供 Windows 便携 ZIP、Android APK 和独立 PWA 更新包。Windows 为未签名测试包，Android 沿用既有测试签名；覆盖安装前请备份笔记。Mac 安装包在独立的 macos-v0.3.5 发行页提供，详见其安装说明。
