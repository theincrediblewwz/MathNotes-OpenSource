...
internal val STANDALONE_FAITHFUL_TRANSCRIPTION_PROMPT = listOf(
    ...
    "图中出现无法用 Markdown 忠实表达的几何图、坐标轴、箭头关系或示意图时，在对应位置简要说明图形内容、标注和它表达的数学关系；紧接说明另起一行只写 [[mathnotes:source-image]]。",
    "软件会把这个固定标记替换为本次实际识别的整张处理后照片。不要框图、给坐标、编造文件地址或链接，也不要恢复裁剪或打码去掉的内容。没有图形时不要添加标记；不要编造原图没有的结论。",
