...
## 0.3.7 文件选择与块结构（候选，尚未发布）

**待完成：真实前台鼠标单击文件夹/ZIP 的验收。自动化运行时应用可能处于后台，键盘选择可用不能替代鼠标验收；在此项确认前不发布 0.3.7。**

已通过 Core 425 项（另 2 项跳过，含分享导入/块恢复 16 项）、原生合同 99 项、UI 合同 205 项和真实预览回归。独立合成库中，Windows 格式 ZIP 与 macOS `ditto` 实际生成的 ZIP 导入后均显示 3 个独立源码块、原图和公式；这些流程使用键盘选择 ZIP；Mac 包曾因未设置 UTF-8 标志导致中文资源名乱码，修复后已用原失败包复测。

分享包选择器改为等待 Notebooks 完成关闭后异步呈现，使用扩展名/目录筛选，并增加主界面 ZIP/文件夹拖入。Windows 默认导出的顶层块标记可恢复独立块的顺序、ID 与来源；代码/公式/引用中的标记不作为边界。Mac 再导出也保留标记。无标记旧包保留全文；连续标记对应的合并续接块缺少切分位置时保留整组并提示，不能声称恢复不存在的锁定或历史元数据。
