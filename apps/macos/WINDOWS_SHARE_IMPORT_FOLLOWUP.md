...
Mac 0.3.7 已支持导入 Windows 导出的分享目录，也可导出 ZIP；解压格式仍是根目录 `<sessionId>.md` 和 `assets/...`。资源 URL 使用已有 `encodeMarkdownAssetPath`，不要再次解码文件名。分享包迁移正文及资源；0.3.7 可从默认导出的顶层 block:id/source 注释恢复普通块的顺序、ID 和来源。锁定、AI 历史或凭据未包含在 Markdown 中；续接组缺少内部偏移时保留合并正文并提示。

Windows 当前 `src/core/exporter.ts` 调用的共享导出器可以继续沿用。后续在 Windows Codex 中完成：

1. 拉取包含 Mac 0.3.7 的 main，保留 Windows 正在开发的工作；不修改 Mac 或 Android 界面。
