...
describe("isTrustedRendererUrl", () => {
  ...
  it("keeps the packaged entrypoint sandboxed with navigation and CSP guards", async () => {
    ...
    const assistantWindowSource = await readFile(path.resolve("src/ui/assistantNativeWindow.ts"), "utf8");
    ...
    expect(mainSource).toContain("isMathNotesAuthorGithubUrl(details.url)");
    expect(mainSource).toContain("shell.openExternal(MATHNOTES_AUTHOR_GITHUB_URL)");
    ...
    expect(mainSource).toMatch(/frameName === "mathnotes-assistant"[\s\S]*?skipTaskbar: false/);
    expect(mainSource).toContain('on("did-create-window"');
    expect(mainSource).toContain('"mathnotes:assistant-window-control"');
    expect(mainSource).toMatch(/assistant-window-control[\s\S]*?action === "restore"[\s\S]*?window\.restore\(\)[\s\S]*?window\.show\(\)[\s\S]*?window\.focus\(\)/);
    expect(appSource).toContain('assistantWindowControl("restore")');
    expect(assistantSource).toContain('className="assistant-workspace-header"');
    expect(assistantSource).toContain('detached ? "detached"');
    ...
    expect(appSource).toContain("openAssistantNativeWindow");
    expect(assistantWindowSource).toContain('opener.open(');
    expect(assistantWindowSource).toContain('assistantWindowName');
