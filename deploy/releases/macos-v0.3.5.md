...
- 修复笔记卡片悬停预览在鼠标移出后残留；预览层不再接收鼠标点击，避免挡住其他卡片。
- 废纸篓的标题固定在顶部，空状态缩小，窗口高度随记录数量调整，较长列表在内容区滚动。
- 废纸篓区分载入、空白和错误状态；新建弹窗统一主题和 Escape 关闭操作。

Mac 版本独立升级到 0.3.5，内置 PWA 仍为最新 0.3.4。原生编译、界面检查及废纸篓实际显示已检查；悬停完整指针路径仍待用户实机确认。
...
下载 `MathNotes-macOS-native-arm64-0.3.5-…-unsigned.zip`，核对同名 SHA-256 文件，解压后将 MathNotes.app 放入 Applications。应用支持声明为 macOS 14 或更新版本；当前实际本机验收为 Apple silicon、macOS 26。Intel Mac 和较早系统尚未实测。
