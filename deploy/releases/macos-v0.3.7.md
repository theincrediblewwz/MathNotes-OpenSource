...
- 分享包文件选择框改为异步打开，等待 Notebooks 窗口关闭后挂到发起操作的工作区窗口，消除关闭动画与阻塞式文件对话框重叠的调用方式。
- 修复 Mac 压缩文件夹后 ZIP 未设置 UTF-8 标志时中文文件名乱码、图片缺失的问题。
- 按文件扩展名与文件夹分别筛选，不依赖 ZIP/Markdown 的系统类型识别；支持直接拖入 ZIP 或分享文件夹到主界面。
- 导入识别 Windows 默认导出的 `<!-- block:id=… source=… -->` 标记，恢复独立内容块、原顺序、有效块 ID 和来源；分隔注释不再作为正文显示。
- Mac 分享 ZIP 导出保留块标记，支持分块结构再次导入；图片仍随包保留。
- 代码示例、数学公式、引用里的类似标记不会被误拆。Windows 合并后的续接块未记录内部切分位置时，保留合并正文并明确提示，不强行拆坏公式或表格。

没有块标记的旧分享包只能保留为完整正文；导出文件未携带的锁定状态、编辑历史等信息无法从 Markdown 恢复。分享包格式仍是 Markdown + assets，正文 8 MB、单资源 64 MB、总包 256 MB。Windows GUI 的带资源分享包回导和真实跨机同步仍待后续实现/验收；本轮不修改 Windows/Android 界面。
...
下载 `MathNotes-macOS-native-arm64-0.3.7-…-unsigned.zip`，核对同名 SHA-256 文件，解压后将 MathNotes.app 放入 Applications。应用支持声明为 macOS 14 或更新版本；当前实际本机验收为 Apple silicon、macOS 26。Intel Mac 和较早系统尚未实测。
