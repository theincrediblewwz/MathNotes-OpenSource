...
export type NotesBackupFile = Readonly<{
  path: string;
  size: number;
  sha256: string;
}>;

export type NotesBackupManifest = Readonly<{
...
export type CreateNotesBackupResult = Readonly<{
  backupDir: string;
  manifestPath: string;
  fileCount: number;
  totalBytes: number;
...
/**
 * Copies only the authoritative notebook tree. Provider credentials, host tokens,
 * logs, settings and process state are intentionally outside this boundary.
 */
...
function assertBackupDestination(notesRootDir: string, destinationParentDir: string): void {
  ...
  if (!destinationRelativeToNotes ||
      (!destinationRelativeToNotes.startsWith("..") && !path.isAbsolute(destinationRelativeToNotes))) {
    throw new Error("备份位置不能位于当前笔记目录内部，请选择其他文件夹。");
  }
}

async function copyTreeWithHashes(
  sourceDir: string,
