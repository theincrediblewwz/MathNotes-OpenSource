...
try {
  ...
  await page.mouse.move(bounds.x+rect.x+rect.width*.35,bounds.y+rect.y+rect.height*.5);await page.mouse.down();
  await page.mouse.move(bounds.x+rect.x+rect.width*.65,bounds.y+rect.y+rect.height*.5,{steps:12});await page.mouse.up();
  check(await page.evaluate(()=>window.qaEdit.marks.some(mark=>mark.type==="redaction")),"redaction gesture did not persist");
  ...
  const report={url,title:await page.title(),pixels,uploaded,afterPerspective,editingDecodeCalls,errors,mobileScreenshot:path.join(output,"pwa-capture-mobile.png"),desktopScreenshot:path.join(output,"pwa-capture-desktop.png")};
